...
public:
    ...
    // 定时器功能
    void runAt(TimeStamp timestamp, std::function<void()> const& callback);
    void runAfter(double wait_time, std::function < void()> const& callback);
    void runEvery(double interval, std::function<void()> const& callback);
private:
