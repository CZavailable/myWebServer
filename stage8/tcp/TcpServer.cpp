...
TcpServer::TcpServer(EventLoop* loop, const char* ip, const int port) : mainReactor(loop) , next_conn_id_(1){ 
    acceptor = std::make_unique<Acceptor>(mainReactor,ip,port);
...
inline void TcpServer::handleCloseInLoop(const std::shared_ptr<TcpConnection>& conn){
    ...
    LOG_INFO << " TcpServer::handleCloseInLoop - Remove connection [id#" <<  conn->getId() << "-fd# " << conn->getFd() << "]";
