...
void EventLoop::runAt(TimeStamp timestamp, std::function<void()> const& callback){
    timer_queue->addTimer(timestamp, std::move(callback), 0.0);
}

void EventLoop::runAfter(double wait_time, std::function<void()> const& callback){
    TimeStamp timestamp(TimeStamp::addTime(TimeStamp::now(), wait_time));
    timer_queue->addTimer(timestamp, std::move(callback), 0.0);
}

void EventLoop::runEvery(double interval, std::function<void()> const& callback){
    TimeStamp timestamp(TimeStamp::addTime(TimeStamp::now(), interval));
    timer_queue->addTimer(timestamp, std::move(callback), interval);
}
